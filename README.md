# 🙋‍♂️ Hi, I'm Rafayel, a QA engineer!

---
## 🌐 Connect with Me:

[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](https://mail.google.com/mail/?view=cm&fs=1&to=rafayel.khachatryan.work@gmail.com) [![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/khachatryanrafayel/)
[![Telegram](https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/rafayel_khach)
[![Instagram](https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/rafayel_khach/)
---
### 👋 About Me

I returned from military service in January 2025 and decided to pursue a career in QA. I often notice usability issues in apps and websites and naturally think of ways to describe and improve them. My problem-solving skills align well with testing techniques like error guessing, and together with my technical background, this gives me a solid start in QA.

Currently, I am developing my skills through self-study and personal projects, focusing on test design, bug reporting, and QA processes. I am seeking an internship or junior QA role to apply my knowledge, contribute to product quality, and grow as a professional.

---

### 🛠 Tech Stack & Skills

| Category          | Technologies/Skills                                                   |
| ----------------- | --------------------------------------------------------------------- |
| **Testing**       | Smoke, Sanity, Regression, Retest                                     |
| **Documentation** | Test Cases, Check-Lists, Bug Reports, TSR (Jira/Trello)               |
| **Databases**     | SQL (queries, subqueries, joins, filtering, aggregates)               |
| **Tools**         | Jira, Trello, Chrome DevTools, Postman, Git, SQL,                     |
| **Languages**     | Python (Basics for automation), c/c++, HTML/CSS (Basic understanding) |

---

### 📁 Projects

Here are my practice projects where I apply my theoretical knowledge.
#### **[ArmTypingTest – Desktop Typing Speed Test Game](https://github.com/KhachatryanRafayel/ArmTypingTest)**
> Pet project for practicing QA skills
- Designed and executed test cases for core functionality (word generation, typing accuracy, result calculation)
- Created checklists for UI/UX elements (real-time feedback, visual cues, result screen)
- Reported and classified bugs found during testing, including:
	- Text input overflowing beyond input field when typing long strings
	- No error indication for incorrect word input (lack of visual feedback)
	- Continuous backspace not functioning properly (only deletes on single press)
- Applied functional and UI testing techniques
- Performed input validation and negative testing (e.g., long text input, continuous backspace behavior)
- Documented test results and defect reports for iterative improvements
---
### 🤝 Let's Connect

I'm always open to discussing new opportunities, internship offers, or collaborative learning projects!

*   **Email:** [rafayel.khachatryan.work@gmail.com](https://mail.google.com/mail/?view=cm&fs=1&to=rafayel.khachatryan.work@gmail.com)
*   **Telegram:** [@rafayel_khach](https://t.me/rafayel_khach)
*   **LinkedIn:** [Rafayel Khachatryan](https://www.linkedin.com/in/khachatryanrafayel/)
---

**Thanks for visiting my profile! Have a great day!**
